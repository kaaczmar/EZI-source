...
{  
   ...
   */
   ...
   private static void r(String s) { if (m() > 0) setto(s); }
   ...
   */
   ...
   private static void step2() { if (ends("y") && vowelinstem()) b[k] = 'i'; }

   /* step3() maps double suffices to single ones. so -ization ( = -ize plus
      -ation) maps to -ize etc. note that the string before the suffix must give
      m() > 0. */

   private static void step3() { if (k == 0) return; /* For Bug 1 */ switch (b[k-1])
   {
   ...
   private static void step4() { switch (b[k])
   {
